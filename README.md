...
Where the decoder is different from encoder; 
decoder can see the words on their left side and the right side is hidden, it will be unidirectional.
In another word, the decoder consider masked self attention mechanism. while encoder consider self attention mechanism.
creat at causal tasks: generation sequance.
